...
class fracturesurgery_env_v2(gym.Env):
    ...
    # Sample 3D bias drift once per episode for 5N scale
       ...
       # fracturestart = np.array([0.3379683792591095, -0.0786883607506752, 0.06340644508600235])- np.array([0.,-0.005,-0.01])
        ...
        #time.sleep(100)
        finger_force_n = 2 if self.soft_tissue=='soft' or self.patient ==132 else 5
       ...
       ## Enable force/torque sensors
        ...
        #leg_start = leg_start - [-0.005,0.00,0.003]#np.array([ 0.32365263, -0.08479164,  0.05334096])
        #leg_start = fracturestart - np.array([0.03,0.07,0.02])
        self.leg = p.loadURDF(leg_path,
                                    basePosition =leg_start,#-[0,1,0],
                                    baseOrientation = leg_orientation,
                                    globalScaling = 1.0,
                                    useFixedBase = 1)
        # print(f"Leg start position: {leg_start}, Leg orientation (quaternion): {leg_orientation}")
        # goal = transformation_matrices.get_goal_from_leg(self, leg_start, leg_orientation)
        # print(f"Goal position: {goal[0]}, Goal orientation (quaternion): {goal[1]}")
        # p.resetBasePositionAndOrientation(self.foot, goal[0], goal[1])
        # print(p.getLinkState(self.foot, 1, computeForwardKinematics=True)[0],p.getLinkState(self.foot, 1, computeForwardKinematics=True)[1])
    ...
    def step(self, action):
        ...
        if self.safe_mode:
            num_steps = utils.compute_cbf_substeps(self.contact_distance, dd_ds=-1.0, nominal_steps=12, max_steps=150)
        ...
        if self.test and (self.filtered_force >= self.max_force or self.isHolding ==0 or self.contact_ema >= self.maximum_contact_force_threshold):
