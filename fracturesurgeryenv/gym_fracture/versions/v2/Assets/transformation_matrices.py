...
def get_leg_start_working(env):
    p_goals ={252: (0.35180669208315357, -0.030067110398995175, 0.07736699857911723), #252
             ...
             198: (0.3595034986898781, -0.03957023388349106, 0.0691244736544874),
...
def get_goal_from_leg(env, leg_pos, leg_ori):
    trans_matrix = get_proximal_to_distal_matrix(env.patient)[0]
    R_mat = trans_matrix[0:3, 0:3]
    t = trans_matrix[0:3, 3]

    # Invert position transformation: p_goal = R_mat * leg_pos + t
    p_goal = R_mat @ leg_pos + t

    # Invert orientation transformation: r_target = r_frame * r_local
    r_frame = R.from_matrix(R_mat.T)
    r_local = R.from_quat(leg_ori)
    r_goal = (r_frame * r_local).as_quat()

    return p_goal, r_goal
...
def get_patient_goal(env, patient_id):
    ...
    goals = {
        ...
        102: (np.array([ 0.30383321, -0.0970693,   0.15603161])-np.array([0.005,-0.0265,0.003]),
              np.array([0.9999999728200057, 0.00023313980271510995, -8.89660707914592e-08, 2.4108688676344187e-06])),
        198:  (np.array([0.3198758237553677, -0.07615291005525722, 0.1462755883802933]) - np.array([0.0,0,0]),#np.array([0.34174003, -0.08506263,  0.16001045]) - np.array([0.01,0.01,0.005])#np.array([ 0.32180062,-0.09246775, 0.15800003]) - np.array([0.005,-0.005,0.00])#np.array([ 0.32180062,-0.09246775, 0.15800003]) - np.array([0.005,-0.005,0.00])
             p.getQuaternionFromEuler(p.getEulerFromQuaternion(p.getLinkState(env.pandaUid, 11)[1])-np.array([6.08/180*np.pi,0, 0]))),
        132: (np.array([0.3121838, -0.08800575, 0.15520251]) - np.array([0.008,-0.032,-0.01]),
